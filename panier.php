    ...
    if (!$erreur){
       switch($action){
          ...
          Case "refresh" :
             ...
             {
                modifierQTeArticle($_SESSION['panier']['nomProduit'][$i],round($QteArticle[$i]),$stock,$annonce);
             }
             break;
    ...
    if (creationPanier())
    {
        ...
        else
        {
            for ($i=0 ;$i < $nbArticles ; $i++)
            {
                ...
                $contenu_panier .= '<img class="image-panier mr-2" src="asset/img_produit/'.htmlspecialchars($_SESSION['panier']['image'][$i]).'" alt="Bungalow" >';
